...
namespace SchoolWebsite.Application.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (password.Equals(confPassword, StringComparison.Ordinal))
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        return RedirectToAction("ManageInsIndex", "Account");
                    }
                }
            }
            return View(viewModel);
        ...
        public IActionResult RemoveLogin(int id)
        {
            ...
            {
                var instructor = queryDb.Instructors.Where(i => i.Id == id).Single();
                ApplicationUser userToDelete = queryDb.Users.Where(u => u.InstructorId == id).Single();
                _userManager.DeleteAsync(userToDelete).Wait();
            }
            catch (Exception)
            {
                ModelState.AddModelError(String.Empty, "Delete of user failed");
            }

            return RedirectToAction("ManageInsIndex");
        }
